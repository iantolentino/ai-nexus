...
if [ -d "$TARGET" ]; then
  ...
  FRAMEWORK_PATHS=(
    ...
    "intents" "modules/MODULE_TEMPLATE.md" "tools/brain-doctor.ps1" "tools/context-metrics.ps1" "tools/select-context.ps1" "tools/context-diff.ps1" "tools/handoff-baseline.ps1" "tools/handoff-compact.ps1" "tools/session-checkpoint.ps1" "tools/slice-file.ps1" "tools/dependency-boundary.ps1" "tools/error-fingerprint.ps1" "tools/record-session-cost.ps1" "tools/skill-router.ps1" "tools/workflow-gate.ps1" "tools/context-decay.ps1" "tools/knowledge-audit.ps1" "tools/token-estimate.ps1" "tools/nexus.ps1" "tools/module-graph.ps1"
  ...
  for path in "fixes/ERROR_FINGERPRINTS.md"; do
    src="$TMP_DIR/_brain/$path"
    dest="$TARGET/$path"
...
echo "Next: open a supported terminal agent in this project and state your task normally."
